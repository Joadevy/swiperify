...
export const getRandomTrackUri = async (spotifyToken:string, amountTracks:number=1) => {
  const randomOffset = Math.floor(Math.random() * 1000);
  const search = getRandomSearch();
  try {
    const track:SpotifyTrackResponse = await fetch(`https://api.spotify.com/v1/search?q=${search}&type=track&limit=${amountTracks}&offset=${randomOffset}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
 ...
 export const playNewSong = async (spotifyToken:string,context_uri:string) => {
